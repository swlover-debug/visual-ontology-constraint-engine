...
import type {
  ...
  ChangeIntent, ConstraintCompilationInput, EffectiveScenario, JsonObject, JsonValue, LocalScenarioPackSource, OutputContract, ProviderCapabilityProfile, ReferenceCandidate, ScenarioPack,
  ScenarioPackManifest, SemanticReviewCriterion, VersionPin,
...
interface FixtureProbeIntent { id: string; operation: ChangeIntent['operation']; targetPath: string; requestedValue?: JsonValue; importance?: ProbeImportance; sourceHintIds?: string[] }
interface FixtureProbeReference { id: string; assetId?: string; role: string; ontologyScopes: string[]; importance: ProbeImportance; sourceBindingIds?: string[]; orderKey?: string }
interface FixtureCoreProbe { id: string; intents?: FixtureProbeIntent[]; unknownPaths?: string[]; waiverTarget?: string; references?: FixtureProbeReference[]; maximumReferenceCount?: number }
...
function pathRelated(left: string, right: string): boolean { return left === right || left.startsWith(`${right}.`) || right.startsWith(`${left}.`) }
function referenceProbeCandidates(probe: FixtureCoreProbe, ir: ReturnType<typeof compileConstraints>): ReferenceCandidate[] {
  return (probe.references ?? []).map((reference) => {
    ...
    const goalIds = ir.goals.filter((goal) => typeof goal.targetPath === 'string' && reference.ontologyScopes.some((scope) => pathRelated(scope, goal.targetPath as string))).map((goal) => goal.id)
    const artifact: ArtifactHandle = { id: reference.assetId ?? reference.id, storeId: 'fixture-store', contentHash: sha256({ fixtureReference: reference.id }), mediaType: 'image/png', byteLength: 64, role: 'reference', resolverId: 'fixture-resolver', availability: 'available', retentionClass: 'fixture', redactionPolicy: 'hash-only' }
...
function semanticCriteriaForScenario(scenario: EffectiveScenario): SemanticReviewCriterion[] {
  const criteria = contributionArray(scenarioContributions(scenario, 'reviewTemplates'), 'criteria').map((criterion) => {
    const id = stringField(criterion, 'id', 'REVIEW_TEMPLATE_INVALID'); const kind = stringField(criterion, 'kind', 'REVIEW_TEMPLATE_INVALID'); const importance = criterion.importance
    if (importance !== 'hard' && importance !== 'required' && importance !== 'preferred') throw new CliError('REVIEW_TEMPLATE_INVALID', 'Review criterion importance is invalid.', EXIT.contract)
    if (criterion.targetPath !== undefined && typeof criterion.targetPath !== 'string') throw new CliError('REVIEW_TEMPLATE_INVALID', 'Review criterion targetPath is invalid.', EXIT.contract)
    if (criterion.prompt !== undefined && typeof criterion.prompt !== 'string') throw new CliError('REVIEW_TEMPLATE_INVALID', 'Review criterion prompt is invalid.', EXIT.contract)
    return { id, kind, importance, ...(criterion.targetPath === undefined ? {} : { targetPath: criterion.targetPath }), ...(criterion.prompt === undefined ? {} : { prompt: criterion.prompt }) } as SemanticReviewCriterion
  })
  const byId = new Map<string, SemanticReviewCriterion>()
  for (const criterion of criteria.sort((left, right) => compare(left.id, right.id))) {
    const prior = byId.get(criterion.id)
    if (prior && canonicalize(prior as unknown as JsonValue) !== canonicalize(criterion as unknown as JsonValue)) throw new CliError('REVIEW_TEMPLATE_ID_COLLISION', 'Review criteria with one ID disagree.', EXIT.contract)
    byId.set(criterion.id, criterion)
  }
  return [...byId.values()]
}
function scenarioEvidence(scenario: EffectiveScenario): JsonObject {
...
function defaultCase(doc: Record<string, unknown>, packId: string, packVersion: string): CaseSpec {
...
async function compileInputs(casePath: string, scenarioPath: string, profilePath: string): Promise<CompileResult> {
  ...
  const caseSpec = defaultCase(caseDoc, loaded.descriptor.manifest.packId, loaded.descriptor.manifest.version); const fixtureId = typeof caseDoc.fixtureId === 'string' ? caseDoc.fixtureId : caseSpec.id
...
async function semanticForRun(run: NonNullable<OfflineExecutionResult['run']>, artifacts: ArtifactHandle[], profile: typeof MOCK_IMAGE_PROFILE, criteria: SemanticReviewCriterion[]): Promise<{ report: import('@voce-engine/contracts').SemanticReviewReport; receipt: import('@voce-engine/contracts').StepReceipt; remote: import('@voce-engine/contracts').RemoteCallRun }> {
...
async function runCommand(args: Record<string, string>): Promise<JsonObject> {
  ...
  const fixture = bundle.payload.fixture as SourceFixtureCase | undefined; const scenario = (bundle.payload.scenario as ResolvedPackResolution | undefined)?.effectiveScenario; const semanticCriteria = scenario ? semanticCriteriaForScenario(scenario) : []; let semanticProposal: import('@voce-engine/contracts').SemanticReviewReport | undefined; const semanticReceipts: import('@voce-engine/contracts').StepReceipt[] = []; const reconciliation: import('@voce-engine/contracts').RemoteCallRun[] = []; if (fixture?.semanticReview) { const semantic = await semanticForRun(run, [structuralArtifact, ...artifacts], profileFor(String((bundle.payload.profile as Record<string, unknown>)?.id ?? MOCK_IMAGE_PROFILE.id)), semanticCriteria); semanticProposal = semantic.report; semanticReceipts.push(semantic.receipt); reconciliation.push(semantic.remote) }
...
async function packTestCommand(source: string): Promise<JsonObject> {
  const loaded = await loadPack(source); const registry = createScenarioPackRegistry(); registry.register(loaded.source); const resolution = registry.resolve({ root: { packId: loaded.descriptor.manifest.packId, versionRange: loaded.descriptor.manifest.version }, extensions: [] }); if (resolution.status !== 'resolved') throw new CliError('PACK_RESOLUTION_BLOCKED', 'ScenarioPack resolution is blocked.', EXIT.contract); const semanticCriteria = semanticCriteriaForScenario(resolution.effectiveScenario)
  const results: JsonObject[] = []
  for (const fixture of loaded.fixtures) {
    const profile = profileFor(fixture.profileId ?? MOCK_IMAGE_PROFILE.id); const caseSpec = defaultCase({ id: fixture.id }, loaded.descriptor.manifest.packId, loaded.descriptor.manifest.version); const input = fixtureM5ExecutionInput(profile); const prompt = fixtureM5Candidate(fixtureM5PromptIR(profile)); const guard = guardPromptCandidate(fixtureM5GuardInput(fixtureM5PromptIR(profile), prompt)); const coreProbes = Object.fromEntries((fixture.coreProbes ?? []).sort((left, right) => compare(left.id, right.id)).map((probe) => [probe.id, coreProbeEvidence(probe, resolution, caseSpec, profile)])); const acceptance: JsonObject = { resolution: { status: resolution.status, selectedCount: resolution.report.selected.length, effectiveScenarioHash: resolution.effectiveScenario.effectiveScenarioHash }, scenario: scenarioEvidence(resolution.effectiveScenario), reference: referenceEvidence(fixture, resolution, caseSpec), coreProbes, compile: { constraintHash: input.constraintIR.deterministicSignature, referencePlanHash: input.referencePlan.planHash, pipelinePlanHash: input.pipelinePlan.planHash, promptGuard: guard.status } }
    ...
    const artifacts = run.outputArtifacts ?? []; const structuralArtifact = fixtureM6Artifact(`structural-${run.id}`, FIXTURE_M6_OPAQUE_PNG, 'image/png', 'generated-image'); const outputContract: OutputContract = { artifactKind: 'image', dataType: 'image', mediaTypes: ['image/png'], cardinality: { min: 1, max: 1 }, background: 'opaque', allowAlpha: false }; const structural = validateStructuralImage({ schemaVersion: 'voce.structural-validation-input/v1alpha1', id: `structural-${run.id}`, artifacts: [{ artifact: structuralArtifact, bytes: FIXTURE_M6_OPAQUE_PNG }], outputContract, expectedCardinality: { min: 1, max: 1 } }); let semanticProposal: import('@voce-engine/contracts').SemanticReviewReport | undefined; const semanticReceipts: import('@voce-engine/contracts').StepReceipt[] = []; const reconciliation: import('@voce-engine/contracts').RemoteCallRun[] = []; if (fixture.semanticReview) { const semantic = await semanticForRun(run, [structuralArtifact, ...artifacts], profile, semanticCriteria); semanticProposal = semantic.report; semanticReceipts.push(semantic.receipt); reconciliation.push(semantic.remote) }
